<script>
	...
	const cx = $derived({
		button: clsx('button inline-flex items-center justify-center shrink-0', children || text ? `p-${size}` : `pz-${size}`, `text-${size}`, {
		...
		text: clsx('button-text line-clamp-1 whitespace-pre', `px-1.5`)
	})
