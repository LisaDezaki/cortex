...
{#snippet item(itemProps, checked)}
	<Flex gap={2} class="input-option px-0.5 py-0.5 w-full {itemProps.className} {checked ? 'active' : ''}" onclick={() => selectItem(itemProps)}>
		{#if itemProps.icon !== undefined}
			<Icon name={itemProps.icon} size="md" />
		{/if}
		{#if itemProps.image !== undefined}
			<Thumbnail class="border-[0.5px] p-[1px] rounded-full w-6" imageClass="rounded-full" iconSize={16} src={itemProps.image} />
		{/if}
		{#if itemProps.label}
			...
			<Grid cols={2} gap={0} class="max-h-96 w-[200%] {suboptions ? 'translate-x-[-50%]' : ''} transition-transform">
				...
				<Stack class="relative max-h-full overflow-hidden">
					<Flex align="center" gap={0} class="bg-white p-0.5 w-full">
						...
						<input bind:value={suboptionQuery} class="border-none px-1 py-1 rounded text-neutral font-style-input w-28" placeholder="Search" />
