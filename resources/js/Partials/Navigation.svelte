...
<Nav>
	...
	{#if project}
		<div class="flex flex-col px-2">
			<!-- <Nav.Item icon="Speedometer"    label="Dashboard"  href="/" 		  active={$page.url === '/'} /> -->
			<Nav.Item icon="UsersThree"     label="Characters" href={route('characters')} 	active={$page.url.startsWith('/characters')} badge={project?.characters?.length} />
			...
			<Nav.Item icon="FlagBannerFold" label="Factions"   href={route('factions')}   	active={$page.url.startsWith('/factions')}   badge={project?.factions?.length} />
			<Nav.Item icon="Backpack"       label="Items"      href="/items"      			active={$page.url.startsWith('/items')}      badge={project?.items?.length}  />
			<Nav.Item icon="MapPinArea"     label="Locations"  href={route('locations')}  	active={$page.url.startsWith('/locations')}  badge={project?.locations?.length}  />
	...
	{#snippet footer()}
		<div class="flex flex-col px-2">
			...
			<Nav.Item icon="ArrowsLeftRight" label="Switch Projects"  href={route('projects')} 	active={$page.url.startsWith('user/settings')} />
			<Nav.Item icon="Plus"   		 label="New Project"      href={route('projects.create')} 	active={$page.url.startsWith('user/settings')} />
